...
/**
 * GraphQL over TCP RSocket integration tests.
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class GraphQLRSocketTCPIT {

    @LocalServerPort
    private int port;

    @Value("${spring.graphql.rsocket.mapping}")
    private String route;

    @Value("${spring.rsocket.server.port}")
    private int portRSocket;
    ...
    void setUp() {
        ...
        TcpClientTransport transport = TcpClientTransport.create(portRSocket);

        RSocketGraphQlTester client = RSocketGraphQlTester.builder()
                .clientTransport(transport)
                .route(route)
                .build();
        this.graphQlTester = client;
    }
